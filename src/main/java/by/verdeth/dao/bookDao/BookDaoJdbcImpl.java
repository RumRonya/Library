...
public class BookDaoJdbcImpl implements BookDao {
    ...
    private final String SQL_SELECT_ALL_BY_ID_SERIA = "SELECT * FROM book WHERE id_author = ?";
    ...
    public List<Book> findAllSortPopular(Integer begin, Integer size) {
        try {
            ...
            PreparedStatement statement = this.connection.prepareStatement("SELECT * FROM book ORDER BY popular DESC LIMIT ? OFFSET ?");
    ...
    public List<Book> findAllSortYear(Integer begin, Integer size) {
        try {
            List<Book> books = new ArrayList();
            PreparedStatement statement = this.connection.prepareStatement("SELECT * FROM book ORDER BY year_book DESC LIMIT ? OFFSET ?");
            statement.setInt(1, size);
            statement.setInt(2, begin);
            ResultSet resultSet = statement.executeQuery();
    ...
    public List<Book> findAllSortDateAdd(Integer begin, Integer size) {
        try {
            List<Book> books = new ArrayList();
            PreparedStatement statement = this.connection.prepareStatement("SELECT * FROM book ORDER BY id_book DESC LIMIT ? OFFSET ?");
            statement.setInt(1, size);
            statement.setInt(2, begin);
            ResultSet resultSet = statement.executeQuery();
    ...
    private Book convertRowResultSetToBook(ResultSet resultSet) {
        try {
            AuthorDao authorDao = new AuthorDaoJdbcImpl(this.dataSource);
            GenreDao genreDao = new GenreDaoJdbcImpl(this.dataSource);
            SeriaDao seriaDao = new SeriaDaoJdbcImpl(this.dataSource);
            Book book = null;
            ...
            Optional<Author> author = authorDao.find(idAuthor);
            Integer idSeria = resultSet.getInt("id_seria");
            Optional<Seria> seria = seriaDao.find(idSeria);
            List<Genre> genres = genreDao.findAllByIdBook(idBook);
            book = new Book(idBook, nameBook, summary, year, URLPoster, popular, author.isPresent() ? (Author)author.get() : null, seria.isPresent() ? (Seria)seria.get() : null, genres);
            return book;
    ...
    public List<Book> findAllByIdAuthor(Integer idAuthor) {
        try {
            List<Book> books = new ArrayList();
            PreparedStatement statement = this.connection.prepareStatement("SELECT * FROM book WHERE id_author = ?");
    ...
    public List<Book> findAllByIdGenre(Integer idGenre) {
        try {
            List<Book> books = new ArrayList();
            PreparedStatement statement = this.connection.prepareStatement("SELECT book.*, genre_of_book.id_book AS book_id FROM book JOIN genre_of_book ON book.id_book = genre_of_book.id_book WHERE genre_of_book.id_genre = ?");
    ...
    public Optional<Book> find(Integer id) {
        try {
            PreparedStatement statement = this.connection.prepareStatement("SELECT * FROM book WHERE id_book = ?");
    ...
    public List<Book> findAll() {
        try {
            List<Book> books = new ArrayList();
            Statement statement = this.connection.createStatement();
            ResultSet resultSet = statement.executeQuery("SELECT book.*, author.id_author as author_id, author.name_author FROM book JOIN author ON book.id_author = author.id_author");
